        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              model.showedMovieAtIndex(index, MovieListType.popular);
...
class _MoviesListDataWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final movie = model.popularMovies[index];
    final posterPath = movie.filmData.posterPath;
    return Material(
      color: Colors.transparent,
      child: InkWell(
        borderRadius: BorderRadius.circular(20),
        onTap: () => model.onMovieTap(context, index, MovieListType.popular),
        child: Padding(
...
class _ScoreWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final score = context.select(
      (NewsViewModel m) => m.popularMovies[index].filmData.voteAverage,
    );
    return Positioned(
      left: 10,
      bottom: 0,
